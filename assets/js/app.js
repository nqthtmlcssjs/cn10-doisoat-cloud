(function(){
  ...
  function applyRoleUI(){
    ...
    if(!canAll()){
      ['#runDiem','#journalDiem','#historyDiem','#lookupDiem','#reportDiem'].forEach(sel=>{ const el=$(sel); if(el){ el.disabled=true; el.value=currentDiemId() || ''; }});
    }
    updateSaveState();
  }
  ...
  async function loadDiemOptions(){
    ...
    ['#historyDiem','#lookupDiem','#reportDiem'].forEach(sel=>{ const el=$(sel); if(el) el.innerHTML=opts; });
  ...
  async function loadDashboardOperational(){
    ...
    const rows=Object.values(by).map(x=>({...x, ty_le:x.tong_ho_so?(x.so_khop/x.tong_ho_so*100):0})).sort((a,b)=>b.so_lech-a.so_lech).slice(0,10);
  ...
  function renderResultTable(rows){
    return table(rows,[{key:'don_vi',label:'Đơn vị'}, {key:'ma_hoso_goc',label:'Mã hồ sơ'}, {key:'ngay_thu',label:'Ngày'}, {key:'thuphi',label:'Thu phí',render:r=>fmt(r.thuphi)}, {key:'bidv',label:'BIDV',render:r=>fmt(r.bidv)}, {key:'sinvoice',label:'SInvoice',render:r=>fmt(r.sinvoice)}, {key:'chenh_bidv',label:'Chênh BIDV',render:r=>fmtDelta(deltaVal(r,'bidv'))}, {key:'chenh_sinvoice',label:'Chênh HĐ',render:r=>fmtDelta(deltaVal(r,'sinvoice'))}, {key:'trang_thai',label:'Trạng thái',render:r=>`<span class="${r.trang_thai==='Matched'?'status-ok':'status-review'}">${escapeHtml(r.trang_thai)}</span>`}, {key:'reason',label:'Loại kiểm tra',render:r=>escapeHtml(reasonLabel(r))}, {key:'tham_chieu_bidv',label:'Tham chiếu'}, {key:'so_hoa_don',label:'Số HĐ'}]);
  }
  ...
  window.CN10_APP={openRun, exportRun, deleteRun, editJournal, deleteJournal, initAuth, getProfileForEmail, loadReport};
