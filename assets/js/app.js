(function(){
  ...
  async function initAuth(){
    ...
    if(currentUser){
      const {data,error}=await supa.from('users_app').select('*, diem(ten_diem)').eq('email', currentUser.email).eq('active', true).maybeSingle();
      if(error) console.error(error);
      if(!data){
        currentProfile = {email:currentUser.email, ho_ten:currentUser.email, vai_tro:'guest', diem_id:null};
        $('#userBadge').textContent = `${currentUser.email} · chưa phân quyền`;
      } else {
        currentProfile = data;
