(function(){
  ...
  $('#refreshBtn')?.addEventListener('click',()=>{loadDashboard(); if($('#history')?.classList.contains('active-view')) loadHistory(); if($('#reports')?.classList.contains('active-view')) loadReport();});
  ...
  async function initAuth(){
    ...
    if(currentUser){
      ...
      if(error){
        ...
        $('#userBadge').textContent = `${currentProfile.ho_ten || email} · ${currentProfile.vai_tro}${suffix}`;
      }
  ...
  function applyRoleUI(){
    ...
    if(!canAll()){
      ['#runDiem','#journalDiem','#historyDiem','#lookupDiem','#reportDiem','#issueDiem'].forEach(sel=>{ const el=$(sel); if(el){ el.disabled=true; el.value=currentDiemId() || ''; }});
    }
    updateSaveState();
  }
  ...
  function updateSaveState(){
    ...
    const hasRole = currentProfile && currentProfile.vai_tro !== 'guest';
    const hasPoint = canAll() ? !!selectedDiem('#runDiem') : !!currentDiemId();
    btn.disabled = !(lastResult && hasRole && hasPoint);
  }
  $('#runDiem')?.addEventListener('change', updateSaveState);
  ...
  async function loadDiemOptions(){
    ...
    ['#historyDiem','#lookupDiem','#reportDiem','#issueDiem'].forEach(sel=>{ const el=$(sel); if(el) el.innerHTML=opts; });
    ['#runDiem','#journalDiem','#userDiem'].forEach(sel=>{ const el=$(sel); if(el) el.innerHTML=optsNoAll; });
    if(currentProfile?.diem_id){ ['#historyDiem','#lookupDiem','#runDiem','#journalDiem','#issueDiem'].forEach(sel=>{ const el=$(sel); if(el) el.value=currentProfile.diem_id; }); }
  }

  function updateKPI(s, runId='-'){
    const total=Number(s?.tong_ho_so||0), matched=Number(s?.so_khop||0), review=Number(s?.so_lech||0);
    const th=Number(s?.tong_thuphi||0), bidv=Number(s?.tong_bidv||0), inv=Number(s?.tong_sinvoice||0);
    const rate = total ? ((matched / total)*100).toFixed(2)+'%' : '0%';
    if($('#kpiTotal')) $('#kpiTotal').textContent=fmt(total)+' hồ sơ';
    if($('#kpiMatched')) $('#kpiMatched').textContent=fmt(matched)+' đã khớp';
    if($('#kpiReview')) $('#kpiReview').textContent=fmt(review);
    if($('#kpiThuPhi')) $('#kpiThuPhi').textContent=fmt(th);
    if($('#kpiBidv')) $('#kpiBidv').textContent=fmt(bidv);
    if($('#kpiSinvoice')) $('#kpiSinvoice').textContent=fmt(inv);
    if($('#kpiDeltaBidv')) $('#kpiDeltaBidv').innerHTML=fmtDelta(th-bidv);
    if($('#kpiDeltaInv')) $('#kpiDeltaInv').innerHTML=fmtDelta(th-inv);
    if($('#kpiRunId')) $('#kpiRunId').textContent='Lần chạy '+runId;
  ...
  async function loadDashboard(){
    ...
    await loadDashboardOperational();
  }


  function classifyIssue(r){
    const th=Number(r.thuphi||0), b=Number(r.bidv||0), inv=Number(r.sinvoice||0);
    if(th!==0 && b===0 && inv===0) return {code:'missing_both', label:'Thiếu BIDV + hóa đơn', level:'danger'};
    if(th!==0 && b===0) return {code:'missing_bidv', label:'Thiếu BIDV', level:'danger'};
    if(th!==0 && inv===0) return {code:'missing_invoice', label:'Thiếu hóa đơn', level:'warn'};
    if(th!==b || th!==inv) return {code:'wrong_amount', label:'Sai tiền', level:'warn'};
    return {code:'review', label:'Cần kiểm tra', level:'warn'};
  }
  function issueBadge(r){ const c=classifyIssue(r); return `<span class="issue-badge ${c.level}">${escapeHtml(c.label)}</span>`; }
  function pointHealthClass(rate){ if(rate>=98) return 'green'; if(rate>=95) return 'orange'; return 'red'; }
  async function loadDashboardOperational(){
    const box=$('#dashboardPointRank'); if(!box) return;
    const ym=new Date().toISOString().slice(0,7); const start=ym+'-01'; const nd=new Date(start+'T00:00:00'); nd.setMonth(nd.getMonth()+1); const end=nd.toISOString().slice(0,10);
    let q=supa.from('dot_doisoat').select('*, diem(ten_diem)').gte('ngay_doisoat',start).lt('ngay_doisoat',end).order('id',{ascending:false}).limit(1000);
    q=scopeQuery(q);
    const {data,error}=await q;
    if(error){ box.innerHTML=`<p class="empty-note">${escapeHtml(error.message)}</p>`; return; }
    const by={};
    for(const r of (data||[])){
      const name=r.diem?.ten_diem || 'CN10';
      if(!by[name]) by[name]={diem:name, so_lan:0, tong_ho_so:0, so_khop:0, so_lech:0, tong_thuphi:0};
      by[name].so_lan++; by[name].tong_ho_so+=Number(r.tong_ho_so||0); by[name].so_khop+=Number(r.so_khop||0); by[name].so_lech+=Number(r.so_lech||0); by[name].tong_thuphi+=Number(r.tong_thuphi||0);
    }
    const rowsAll=Object.values(by).map(x=>({...x, ty_le:x.tong_ho_so?(x.so_khop/x.tong_ho_so*100):0})).sort((a,b)=>b.so_lech-a.so_lech);
    const rows=rowsAll.slice(0,10);
    box.innerHTML=table(rows,[{key:'diem',label:'Điểm'}, {key:'so_lan',label:'Số lần',render:r=>fmt(r.so_lan)}, {key:'tong_ho_so',label:'Hồ sơ',render:r=>fmt(r.tong_ho_so)}, {key:'so_khop',label:'Khớp',render:r=>fmt(r.so_khop)}, {key:'so_lech',label:'Cần xử lý',render:r=>`<b class="${Number(r.so_lech)>0?'delta-danger':'delta-ok'}">${fmt(r.so_lech)}</b>`}, {key:'ty_le',label:'Tỷ lệ',render:r=>Number(r.ty_le||0).toFixed(2)+'%'}, {key:'tong_thuphi',label:'Thu phí',render:r=>fmt(r.tong_thuphi)}], 'Chưa có dữ liệu trong tháng.');
    renderPointMap(rowsAll);
    loadIssueQueuePreview();
  }
  function renderPointMap(rowsAll){
    const map=$('#pointMap'); if(!map) return;
    const byName=Object.fromEntries((rowsAll||[]).map(x=>[x.diem,x]));
    let missing=0;
    map.innerHTML=(allDiem||[]).map(d=>{
      const r=byName[d.ten_diem];
      if(!r) missing++;
      const rate=r?.ty_le ?? 0;
      const cls=r?pointHealthClass(rate):'gray';
      const sub=r?`${rate.toFixed(1)}% · ${fmt(r.so_lech)} lỗi`:'Chưa đối soát';
      return `<button class="point-pill ${cls}" title="${escapeHtml(d.ten_diem)}"><b>${escapeHtml(d.ten_diem)}</b><small>${sub}</small></button>`;
    }).join('');
    if($('#kpiMissingPoint')) $('#kpiMissingPoint').textContent=fmt(missing);
  }
  ...
  async function deleteRun(id){
    ...
    if(error) alert(error.message); else { await logAction('DELETE_RUN', {dot_id:id}); alert('Đã xóa'); loadDashboard(); loadHistory(); $('#historyDetail').classList.add('hidden'); }
  }
  ...
  $('#runBtn')?.addEventListener('click', async()=>{
    ...
    try{
      ...
      updateKPI(lastResult.summary, '-'); renderCurrentTab(); $('#exportBtn').disabled=false; updateSaveState();
      const needPoint = canAll() && !selectedDiem('#runDiem') ? ' Chọn điểm trước khi lưu Supabase.' : '';
      msg($('#runMessage'),`Đã chạy xong: ${fmt(lastResult.summary.tong_ho_so)} hồ sơ, khớp ${fmt(lastResult.summary.so_khop)}, lệch ${fmt(lastResult.summary.so_lech)}.${needPoint}`, !needPoint);
      showView('reconcile');
  ...
  function reasonLabel(r){
    if(r.trang_thai==='Matched') return 'Khớp';
    const th=Number(r.thuphi||0), b=Number(r.bidv||0), s=Number(r.sinvoice||0);
    if(th!==0 && b===0 && s===0) return 'Thiếu BIDV + hóa đơn';
    if(th!==0 && b===0) return 'Thiếu BIDV';
    if(th!==0 && s===0) return 'Thiếu hóa đơn';
    if(th!==b || th!==s) return 'Sai tiền';
    return 'Cần kiểm tra';
  }
  ...
  async function saveSupabase(){
    ...
    try{
      ...
      await logAction('SAVE_RUN', {dot_id:dotId, file_name:lastFileName, diem_id:diemId});
  ...
  $('#reportDiem')?.addEventListener('change', loadReport);
  $('#reportMonth')?.addEventListener('change', loadReport);
  $('#reportExportBtn')?.addEventListener('click', exportReport);
  let currentReport = {rows:[], runs:[], summary:{}};

  function monthRange(ym){
    const val = ym || new Date().toISOString().slice(0,7);
    const start = val + '-01';
    const d = new Date(start + 'T00:00:00');
    d.setMonth(d.getMonth()+1);
    const end = d.toISOString().slice(0,10);
    return {start, end, label:val};
  }
  async function loadReport(){
    const elK=$('#reportKpis'), elP=$('#reportByPoint'), elR=$('#reportRuns');
    if(!elK || !elP || !elR) return;
    const {start,end,label}=monthRange($('#reportMonth')?.value);
    let q=supa.from('dot_doisoat').select('*, diem(ten_diem,ma_diem)').gte('ngay_doisoat', start).lt('ngay_doisoat', end).order('ngay_doisoat',{ascending:false}).order('id',{ascending:false}).limit(1000);
    q=scopeQuery(q,'#reportDiem');
    const {data,error}=await q;
    if(error){ elP.innerHTML=`<p class="empty-note">${escapeHtml(error.message)}</p>`; return; }
    const runs=data||[];
    const by={};
    for(const r of runs){
      const key=r.diem_id || 'all';
      const name=r.diem?.ten_diem || 'CN10';
      if(!by[key]) by[key]={diem:name, so_lan:0, tong_ho_so:0, so_khop:0, so_lech:0, tong_thuphi:0, tong_bidv:0, tong_sinvoice:0};
      by[key].so_lan += 1;
      by[key].tong_ho_so += Number(r.tong_ho_so||0);
      by[key].so_khop += Number(r.so_khop||0);
      by[key].so_lech += Number(r.so_lech||0);
      by[key].tong_thuphi += Number(r.tong_thuphi||0);
      by[key].tong_bidv += Number(r.tong_bidv||0);
      by[key].tong_sinvoice += Number(r.tong_sinvoice||0);
    }
    const rows=Object.values(by).map(x=>({...x, ty_le:x.tong_ho_so?((x.so_khop/x.tong_ho_so)*100):0})).sort((a,b)=>b.so_lech-a.so_lech);
    const sum=rows.reduce((a,x)=>{ for(const k of ['so_lan','tong_ho_so','so_khop','so_lech','tong_thuphi','tong_bidv','tong_sinvoice']) a[k]=(a[k]||0)+Number(x[k]||0); return a; },{});
    currentReport={rows,runs,summary:sum,label};
    elK.innerHTML=`<div class="kpi-card"><span>Tháng</span><strong>${escapeHtml(label)}</strong></div><div class="kpi-card"><span>Số lần chạy</span><strong>${fmt(sum.so_lan)}</strong></div><div class="kpi-card"><span>Tổng hồ sơ</span><strong>${fmt(sum.tong_ho_so)}</strong></div><div class="kpi-card ok"><span>Đã khớp</span><strong>${fmt(sum.so_khop)}</strong></div><div class="kpi-card warn"><span>Cần xử lý</span><strong>${fmt(sum.so_lech)}</strong></div><div class="kpi-card ok"><span>Tỷ lệ khớp</span><strong>${sum.tong_ho_so?((sum.so_khop/sum.tong_ho_so)*100).toFixed(2)+'%':'0%'}</strong></div>`;
    elP.innerHTML=table(rows,[{key:'diem',label:'Điểm'}, {key:'so_lan',label:'Số lần',render:r=>fmt(r.so_lan)}, {key:'tong_ho_so',label:'Hồ sơ',render:r=>fmt(r.tong_ho_so)}, {key:'so_khop',label:'Khớp',render:r=>fmt(r.so_khop)}, {key:'so_lech',label:'Lệch',render:r=>fmt(r.so_lech)}, {key:'ty_le',label:'Tỷ lệ',render:r=>Number(r.ty_le||0).toFixed(2)+'%'}, {key:'tong_thuphi',label:'Thu phí',render:r=>fmt(r.tong_thuphi)}, {key:'tong_bidv',label:'BIDV',render:r=>fmt(r.tong_bidv)}, {key:'tong_sinvoice',label:'SInvoice',render:r=>fmt(r.tong_sinvoice)}]);
    elR.innerHTML=renderRuns(runs);
  }
  function exportReport(){
    if(!currentReport.rows.length){ alert('Chưa có dữ liệu báo cáo'); return; }
    const wb=XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(currentReport.rows), 'TongHopTheoDiem');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(currentReport.runs), 'ChiTietLanChay');
    XLSX.writeFile(wb, `bao_cao_doi_soat_cn10_${currentReport.label||'thang'}.xlsx`);
  }



  let currentIssues=[];
  $('#issueLoadBtn')?.addEventListener('click', loadIssues);
  $('#issueDiem')?.addEventListener('change', loadIssues);
  $('#issueType')?.addEventListener('change', loadIssues);
  $('#issueKeyword')?.addEventListener('keydown', e=>{ if(e.key==='Enter') loadIssues(); });
  $('#issueExportBtn')?.addEventListener('click', exportIssues);

  async function loadIssueQueuePreview(){
    const el=$('#issueQueuePreview'); if(!el) return;
    const rows=await fetchIssues({limit:8});
    el.innerHTML=renderIssues(rows, true);
  }
  async function loadIssues(){
    const rows=await fetchIssues({limit:1000});
    currentIssues=rows;
    const by={}; rows.forEach(r=>{ const c=classifyIssue(r).label; by[c]=(by[c]||0)+1; });
    const s=Object.entries(by).map(([k,v])=>`<div class="kpi-card"><span>${escapeHtml(k)}</span><strong>${fmt(v)}</strong></div>`).join('') || '<p class="empty-note">Không có lỗi cần xử lý.</p>';
    if($('#issueSummary')) $('#issueSummary').innerHTML=s;
    if($('#issueTable')) $('#issueTable').innerHTML=renderIssues(rows, false);
  }
  async function fetchIssues({limit=500}={}){
    let q=supa.from('ketqua_doisoat').select('*, dot_doisoat(ngay_doisoat,file_name,diem_id)').neq('trang_thai','Matched').order('id',{ascending:false}).limit(limit);
    q=scopeQuery(q,'#issueDiem');
    const {data,error}=await q;
    if(error){ console.error(error); return []; }
    let rows=data||[];
    const typ=$('#issueType')?.value || 'all';
    if(typ!=='all') rows=rows.filter(r=>classifyIssue(r).code===typ);
    const kw=($('#issueKeyword')?.value||'').trim().toLowerCase();
    if(kw) rows=rows.filter(r=>[r.ma_hoso_goc,r.ma_hoso_sach,r.so_hoa_don,r.tham_chieu_bidv,r.don_vi].some(x=>String(x||'').toLowerCase().includes(kw)));
    return rows;
  }
  function renderIssues(rows, compact=false){
    return table(rows,[
      {key:'loai',label:'Loại lỗi',render:r=>issueBadge(r)},
      {key:'ma_hoso_goc',label:'Mã hồ sơ'},
      {key:'don_vi',label:'Đơn vị'},
      {key:'thuphi',label:'Thu phí',render:r=>fmt(r.thuphi)},
      {key:'bidv',label:'BIDV',render:r=>fmt(r.bidv)},
      {key:'sinvoice',label:'SInvoice',render:r=>fmt(r.sinvoice)},
      {key:'chenh_bidv',label:'Chênh BIDV',render:r=>fmtDelta(deltaVal(r,'bidv'))},
      {key:'chenh_sinvoice',label:'Chênh HĐ',render:r=>fmtDelta(deltaVal(r,'sinvoice'))},
      {key:'act',label:'Xử lý',render:r=>`<button class="mini-btn" onclick="CN10_APP.createJournalFromIssue('${escapeHtml(r.ma_hoso_goc||'')}', '${escapeHtml(r.tham_chieu_bidv||r.so_hoa_don||'')}', '${escapeHtml(classifyIssue(r).label)}')">Tạo nhật ký</button>`}
    ], compact?'Không có lỗi trong hàng đợi.':'Không có lỗi phù hợp bộ lọc.');
  }
  function createJournalFromIssue(ma, ref, note){
    showView('journal');
    if($('#journalMa')) $('#journalMa').value=ma||'';
    if($('#journalRef')) $('#journalRef').value=ref||'';
    if($('#journalNote')) $('#journalNote').value=`Từ hàng đợi lỗi: ${note||''}`;
    if(ref && /^\d+$/.test(String(ref))) $('#journalType').value='sinvoid'; else $('#journalType').value='saoke';
    window.scrollTo({top:0,behavior:'smooth'});
  }
  function exportIssues(){
    if(!currentIssues.length){ alert('Chưa có dữ liệu lỗi để xuất'); return; }
    const wb=XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(currentIssues.map(r=>({...r, loai_loi:classifyIssue(r).label}))), 'HangDoiXuLy');
    XLSX.writeFile(wb, `hang_doi_xu_ly_cn10_${new Date().toISOString().slice(0,10)}.xlsx`);
  }
  ...
  async function logAction(action, detail={}){
    try{
      await supa.from('audit_log').insert({
        ...
        vai_tro: currentProfile?.vai_tro || null,
        diem_id: detail.diem_id || currentProfile?.diem_id || null,
        detail
      });
    }catch(e){ console.warn('audit_log skipped:', e.message); }
  }
  window.CN10_APP={openRun, exportRun, deleteRun, editJournal, deleteJournal, initAuth, getProfileForEmail, loadReport, showView, createJournalFromIssue};
