-- CN10 Enterprise 1.0.0 - Install / Verify
-- Chạy được trên Supabase project sạch. Nếu đã chạy Sprint 1-4A thì file này chỉ bổ sung phần thiếu.
...
values (1,'1.0.0-enterprise','1.0.0-enterprise','1.0.0-enterprise','CN10 Enterprise Fee Control Center','Bản chạy thử chính thức')
...
on conflict(id) do update set
  ten_diem=excluded.ten_diem, ma_diem=excluded.ma_diem, thu_tu=excluded.thu_tu, active=true;
...
create index if not exists idx_bank_session on bank_unidentified(session_id);
create index if not exists idx_invoice_session on invoice_unidentified(session_id);
